...
** SELECT ALL: +start n=node(0) match (n)-[:TYPE]->(type {type:$$TABLE$$})<-[:IS_A]-(instance) return instance 
+
** SELECT COUNT: +start n=node(0) match (n)-[:TYPE]->(type {type:$$TABLE$$})<-[:IS_A]-(instance) return count(instance)+
